...
class CommLayer(nn.Module):
    def __init__(self, input_dim=config.hidden_dim, message_dim=32, pos_embed_dim=16, num_heads=4):
        ...
        self.message_key = nn.Linear(
            input_dim+pos_embed_dim, message_dim * num_heads)
        self.message_value = nn.Linear(
            input_dim+pos_embed_dim, message_dim * num_heads)
        self.hidden_query = nn.Linear(input_dim, message_dim * num_heads)

        self.head_agg = nn.Linear(
            message_dim * num_heads, message_dim * num_heads)
    ...
    def position_embed(self, relative_pos, dtype, device):
        ...
        relative_pos = relative_pos.reshape(
            batch_size*num_agents*num_agents, 2)
        relative_pos_idx = relative_pos[:, 0] + relative_pos[:, 1]*9

        one_hot_position[torch.arange(
            batch_size*num_agents*num_agents), relative_pos_idx.long()] = 1
    ...
    def forward(self, hidden, relative_pos, comm_mask):
        ...
        # attention
        # batch_size x num_agents x num_agents x self.num_heads x 1
        attn_score = (hidden_q * message_k).sum(4,
                                                keepdim=True) / self.message_dim**0.5
        attn_score.masked_fill_(attn_mask, torch.finfo(attn_score.dtype).min)
        attn_weights = F.softmax(attn_score, dim=1)

        # agg
        agg_message = (message_v * attn_weights).sum(1).view(batch_size,
                                                             num_agents, self.num_heads*self.message_dim)
...
class CommBlock(nn.Module):
    ...
    def forward(self, latent, relative_pos, comm_mask):
        ...
        assert relative_pos.size() == (batch_size, num_agents,
                                       num_agents, 2), relative_pos.size()
...
class Network(nn.Module):
    ...
    def step(self, obs, last_act, pos):
        ...
        if self.selective_comm:
            ...
            for i in range(num_agents-1):
                origin_agent_idx[i+1] = test_mask[i, i:].sum() + \
                    test_mask[i+1, :i+1].sum() + origin_agent_idx[i]
            test_obs = torch.repeat_interleave(obs, num_agents, dim=0).view(
                num_agents, num_agents, *config.obs_shape)[test_mask]

            test_relative_pos = relative_pos[test_mask]
            test_relative_pos += config.obs_radius

            test_obs[torch.arange(num_in_obs_agents.sum(
            )), 0, test_relative_pos[:, 0], test_relative_pos[:, 1]] = 0

            test_last_act = torch.repeat_interleave(
                last_act, num_in_obs_agents, dim=0)
            if self.hidden is None:
                test_hidden = torch.zeros(
                    (num_in_obs_agents.sum(), self.hidden_dim))
            else:
            ...
            actions_mat = torch.ones(
                (num_agents, num_agents), dtype=test_actions.dtype) * -1
            actions_mat[test_mask] = test_actions
            diff_action_mask = actions_mat != actions_mat[agent_indexing, agent_indexing].unsqueeze(
                1)
        ...
        self.hidden = self.comm(self.hidden.unsqueeze(
            0), relative_pos.unsqueeze(0), comm_mask.unsqueeze(0))
    ...
    def forward(self, obs, last_act, steps, hidden, relative_pos, comm_mask):
        ...
        last_act = last_act.view(
            seq_len*batch_size*num_agents, config.action_dim)
