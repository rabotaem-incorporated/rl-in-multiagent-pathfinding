...
class SumTree:
    ...
    def batch_sample(self, batch_size: int):
        ...
        assert np.all(priorities > 0), 'idx: {}, priority: {}'.format(
            idxes, priorities)
...
class LocalBuffer:
    ...
    def finish(self, last_q_val=None, last_relative_pos=None, last_comm_mask=None):
        ...
        if last_q_val is None:
            ...
            self.relative_pos_buf[self.burn_in_steps +
                                  self.size] = last_relative_pos
        ...
        td_errors[:self.size] = np.abs(
            self.rew_buf+max_q*cumulated_gamma-target_q).clip(1e-6)
        sizes = np.array([min(self.chunk_capacity, self.size-i*self.chunk_capacity)
                         for i in range(num_chunks)], dtype=np.uint8)
