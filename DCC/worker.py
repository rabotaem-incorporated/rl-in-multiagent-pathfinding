    ...
    def add(self, data: EpisodeData):
        ...
        with self.lock:
            ...
            for i, size in enumerate(data.sizes):
                idxes = np.arange(self.ptr*self.chunk_capacity,
                                  (self.ptr+1)*self.chunk_capacity)
                start_idx = self.ptr*self.chunk_capacity
                # update buffer size
                self.counter += size

                self.priority_tree.batch_update(
                    idxes, data.td_errors[i*self.chunk_capacity:(i+1)*self.chunk_capacity]**self.alpha)

                self.obs_buf[self.ptr] = np.copy(data.obs[i*self.chunk_capacity:(
                    i+1)*self.chunk_capacity+config.burn_in_steps+config.forward_steps])
                self.last_act_buf[self.ptr] = np.copy(data.last_act[i*self.chunk_capacity:(
                    i+1)*self.chunk_capacity+config.burn_in_steps+config.forward_steps])
                self.act_buf[start_idx:start_idx+size] = data.actions[i *
                                                                      self.chunk_capacity:i*self.chunk_capacity+size]
                self.rew_buf[start_idx:start_idx+size] = data.rewards[i *
                                                                      self.chunk_capacity:i*self.chunk_capacity+size]
                self.hid_buf[self.ptr] = np.copy(
                    data.hiddens[i*self.chunk_capacity:i*self.chunk_capacity+size+config.forward_steps])
                self.size_buf[self.ptr] = size
                self.relative_pos_buf[self.ptr] = np.copy(data.relative_pos[i*self.chunk_capacity:(
                    i+1)*self.chunk_capacity+config.burn_in_steps+config.forward_steps])
                self.comm_mask_buf[self.ptr] = np.copy(data.comm_mask[i*self.chunk_capacity:(
                    i+1)*self.chunk_capacity+config.burn_in_steps+config.forward_steps])
                self.gamma_buf[start_idx:start_idx+size] = data.gammas[i *
                                                                       self.chunk_capacity:i*self.chunk_capacity+size]
    ...
    def _sample_batch(self, batch_size: int) -> Tuple:
        ...
        with self.lock:
            ...
            for global_idx, local_idx in zip(global_idxes.tolist(), local_idxes.tolist()):
                ...
                relative_pos = self.relative_pos_buf[global_idx][local_idx:local_idx +
                                                                 config.burn_in_steps+steps+1]
                comm_mask = self.comm_mask_buf[global_idx][local_idx:local_idx +
                                                           config.burn_in_steps+steps+1]
                obs = self.obs_buf[global_idx][local_idx:local_idx +
                                               config.burn_in_steps+steps+1]
                last_act = self.last_act_buf[global_idx][local_idx:local_idx +
                                                         config.burn_in_steps+steps+1]
            ...
            data = (
                ...
                torch.from_numpy(np.stack(b_last_act)).transpose(
                    1, 0).contiguous(),
    ...
    def update_priorities(self, idxes: np.ndarray, priorities: np.ndarray, old_ptr: int):
        ...
        with self.lock:
            ...
            if self.ptr > old_ptr:
                ...
                mask = (idxes < old_ptr*self.chunk_capacity) | (idxes >=
                                                                self.ptr*self.chunk_capacity)
                idxes = idxes[mask]
        ...
        for num_agents in range(config.init_env_settings[0], config.max_num_agents+1):
            ...
            for map_len in range(config.init_env_settings[1], config.max_map_lenght+1, 5):
                if (num_agents, map_len) in self.stat_dict:
                    print('{:4d}/{:<3d}'.format(sum(self.stat_dict[(num_agents, map_len)]), len(
                        self.stat_dict[(num_agents, map_len)])), end='')
                else:
                    print('   N/A  ', end='')
            print()
...
class Learner:
    ...
    def _train(self):
        ...
        for i in range(1, config.training_steps+1):
            ...
            b_obs, b_last_act, b_action, b_reward = b_obs.to(self.device), b_last_act.to(
                self.device), b_action.to(self.device), b_reward.to(self.device)
            ...
            b_relative_pos, b_comm_mask = b_relative_pos.to(
                self.device), b_comm_mask.to(self.device)
            ...
            with torch.no_grad():
                b_q_ = self.tar_model(b_obs, b_last_act, b_next_seq_len, b_hidden,
                                      b_relative_pos, b_comm_mask).max(1, keepdim=True)[0]

            target_q = b_reward + b_gamma * b_q_

            b_q = self.model(b_obs[:-config.forward_steps], b_last_act[:-config.forward_steps], b_seq_len, b_hidden,
                             b_relative_pos[:, :-config.forward_steps], b_comm_mask[:, :-config.forward_steps]).gather(1, b_action)
            ...
            nn.utils.clip_grad_norm_(
                self.model.parameters(), config.grad_norm_dqn)
            ...
            if i % config.save_interval == 0:
                torch.save(self.model.state_dict(), os.path.join(
                    config.save_path, f'{self.counter}.pth'))
    ...
    def stats(self, interval: int):
        ...
        if self.counter != self.last_counter:
            print('loss: {:.4f}'.format(
                self.loss/(self.counter-self.last_counter)))
...
class Actor:
    ...
    def run(self):
        ...
        while True:
            ...
            local_buffer.add(q_val[0], actions[0], last_act,
                             rewards[0], next_obs, hidden, relative_pos, comm_mask)
            ...
            if done == False and self.env.steps < self.max_episode_length:
                ...
                if done:
                    ...
                    data = local_buffer.finish(
                        q_val[0], relative_pos, comm_mask)
    ...
    def _update_weights(self):
        ...
        new_env_settings_set = ray.get(
            self.global_buffer.get_env_settings.remote())
