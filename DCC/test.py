...
def create_test(
    test_env_settings: Tuple = config.test_env_settings,
    num_test_cases: int = config.num_test_cases,
):
    """
    create test set
    """
...
def test_model(
    model_range: Union[int, tuple], test_set: Tuple = config.test_env_settings
):
    """
    ...
    if isinstance(model_range, int):
        state_dict = torch.load(
            os.path.join(config.save_path, f"{model_range}.pth"),
            map_location=DEVICE,
            weights_only=False,
        )
        ...
        for case in test_set:
            ...
            with open(
                "./test_set/{}length_{}agents_{}density.pth".format(
        ...
        for model_name in range(
            model_range[0], model_range[1] + 1, config.save_interval
        ):
            state_dict = torch.load(
                os.path.join(config.save_path, f"{model_name}.pth"), map_location=DEVICE
            )
            network.load_state_dict(state_dict)
            network.eval()
            network.share_memory()
            ...
            for case in test_set:
                print(f"test set: {case[0]} length {case[1]} agents {case[2]} density")
                with open(
                    f"./test_set/{case[0]}length_{case[1]}agents_{case[2]}density.pth",
                    "rb",
                ) as f:
                    tests = pickle.load(f)
...
def test_one_case_and_animate(args, path):
    env_set, network = args
    env = Environment()
    env.load(env_set[0], env_set[1], env_set[2])
    obs, last_act, pos = env.observe()
    ...
    while not done and env.steps < config.max_episode_length:
        ...
        for i in range(len(actions)):
            if delta[i][0] == 0 and delta[i][1] == 0:
                real_actions[i] = 4
            if delta[i][0] == -1 and delta[i][1] == 0:
                real_actions[i] = 0
            if delta[i][0] == 1 and delta[i][1] == 0:
                real_actions[i] = 1
            if delta[i][0] == 0 and delta[i][1] == -1:
                real_actions[i] = 2
            if delta[i][0] == 0 and delta[i][1] == 1:
                real_actions[i] = 3
        prev_pos = cur_pos
...
if __name__ == "__main__":
    os.chdir(os.path.join(os.getcwd(), "DCC"))
    ...
    tests = [(test, network) for test in tests]
    test_one_case_and_animate(tests[0], path="../test_visualizer/animation.svg")
